...
class Ec2Inventory(object):
    ...
    def add_instance(self, instance, region):
        ...
        self.push(self.inventory, instance.vpc_id, dest)
    ...
    def get_host_info_dict_from_instance(self, instance):
        instance_vars = {}
        c = boto.vpc.connect_to_region(instance.region.name)
        vpc = c.get_all_vpcs(vpc_ids=instance.vpc_id)[0]
        for key in vars(vpc):
            value = getattr(vpc, key)
            key = self.to_safe('vpc_' + key)
            if key == 'vpc_tags':
